  ...
  try {
    ...
    if (String(body.section) === 'social') {
      const system = [
        ...
        'short: a single post under 240 characters including the link, for X, Bluesky, and Mastodon. Structure, pick ONLY the single strongest item, open with it as a hook (a striking number or surprising fact works best), add one line on why it matters to a normal person, then the link written as https://www.odipa.org/newsletter so it works everywhere. End with 1 or 2 topical hashtags such as #privacy or #databreach, chosen to match the item, still within the character limit. Never list multiple items, never write a summary of the whole issue. It should make someone stop scrolling, not inform them of everything.',
      ].join(' ')
