...
function assembleHtml(month: string, sections: { heading: string; items: Item[] }[]) {
  ...
  for (const sec of sections) {
    const items = realItems(sec.items)
...
export default function NewsletterAdminDashboard() {
  ...
  const sections = [
    ...
    { heading: 'Tools That Help', items: toolPicks },
  ]
  ...
  async function generateDraft() {
    ...
    const gathered: Record<string, Item[]> = {}
    ...
    // With the news in hand, ask for tool recommendations from ODIPA's real catalog
    const newsContext = [...(gathered.breaches || []), ...(gathered.laws || []), ...(gathered.tips || [])]
      .map(i => i.title).join('; ')
    if (newsContext) {
      try {
        const d = await adminPost('/api/newsletter-generate', { section: 'toolpicks', month, context: newsContext })
        setToolPicks(d.items || [])
      } catch { failed.push('tool picks') }
    }
      ...
      <div className="max-w-5xl mx-auto px-6 py-10 space-y-6">
        ...
        {key.trim() && (
          <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
            ...
            <SectionEditor heading="Tools That Help" items={toolPicks} setItems={setToolPicks}
              hint="AI picks 2 or 3 tools from ODIPA's real directory that fit this month's news. Only catalog tools are allowed, edit freely." />
